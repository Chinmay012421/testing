...
def init_db():
    ...
    # Add optional report fields to older databases without breaking them.
    extra_columns = {
        "feedback_type": "TEXT DEFAULT 'Complaint'",
...
def save_report(username, category, location, message, date,
                feedback_type="Complaint", student_name="", student_class="",
                priority="Normal", subject="", anonymous=False):
    conn = sqlite3.connect(DATABASE)
    cursor = conn.cursor()
    ...
    cursor.execute("""
        ...
        (username, category, location, message, status, date, admin_note,
         feedback_type, student_name, student_class, priority, subject, anonymous)
        VALUES (?, ?, ?, ?, 'Pending', ?, '', ?, ?, ?, ?, ?, ?)
    """, (username, category, location, message, date, feedback_type,
          student_name, student_class, priority, subject, int(bool(anonymous))))
